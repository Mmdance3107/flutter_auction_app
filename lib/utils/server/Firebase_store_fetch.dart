  ...
  Future<void> placeBid(String productID,
    ...
    try {
      ...
      if (bidAmount <= currentBalance) {
        ...
        //FirebaseFirestore.instance.collection('Users').doc().collection('Bids').doc().set(bidData);
  ...
  Future<String?> updateStatusToCompleted(String documentID) async {
    try {
      ...
      for (var bidDoc in bidsSnapshot.docs) {
        double bidPrice = bidDoc['Bidding_price']; //double bidPrice = bidDoc['Bidding_price'];
      ...
      else if(highestBidPrice == 0){
        await auctionsCollection.doc(documentID).update({
          'status': 'ended',
          'winner': 'nobody',
        });

        print('Status updated to Ended successfully');
        return winner;
      }
      else {
  ...
  Future<List<Map<String, dynamic>>> fetchProductsAll() async {
    try {
      QuerySnapshot querySnapshot = await FirebaseFirestore.instance
          .collection('Auctions').where('winner', isNotEqualTo: 'nobody')
          .get();
